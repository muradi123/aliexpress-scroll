  ...
  if (url.includes("aliexpress.com") && (url.includes("orderList") || url.includes("order_list"))) {
  let currentPage = Number($($('.ui-label')[0]).text().split("/")[0]);
  let nextPage = currentPage + 1;
  let numberOfAdd = 0;
  let noPages = 0;

  if (numberOfAdd === 0 && noPages === 0 && nextPage !== currentPage) {
      if (scroll > 0.60 * height) {
        numberOfAdd = 1;
      }};   
  }
  ...
  $.ajax({
    ...
    if (list[i].adTag || list.adSessionId) {
        continue;
    }
    let item = list[i];
